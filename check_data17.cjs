...
async function run() {
  ...
  const { data: people } = await supabase.from('people').select('id, name, team, employment_start_date, employment_end_date, overall_start_date, overall_end_date');
  ...
  console.log("Top 10 highest logged hours among CURRENT employees:");
  const currentPeople = Array.from(deduped.values()).filter(p => !p.hasEnded);
  currentPeople.sort((a,b) => b.actualHours - a.actualHours);
  ...
  for (const p of currentPeople.slice(0, 10)) {
     console.log(`${p.name}: Actual=${p.actualHours}, Expected=${p.expectedTotalHours}`);
  }
  
  console.log(`\nCurrent Only (Count=${currentCount}): Expected=${currentExpected}, Actual=${currentActual}`);
}
run();
