-- Update get_project_costs to handle weekly capacity correctly
        ...
        WHEN p.annual_salary IS NOT NULL AND p.annual_salary > 0
          ...
          te.hours * (
            ...
            NULLIF(1665.0 * (
              COALESCE(
                CASE 
                  WHEN r.billable_capacity_hours > 12 THEN r.billable_capacity_hours / 5.0 
                  ELSE NULLIF(r.billable_capacity_hours, 0) 
                END, 
                7.5
              ) / 7.5
            ), 0)
...
-- Update get_project_costs_monthly to handle weekly capacity correctly
...
-- Update get_project_costs_by_role to handle weekly capacity correctly
