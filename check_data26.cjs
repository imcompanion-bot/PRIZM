...
async function run() {
  ...
  const { data: pData } = await supabase.from('parental_leave').select('*');
  const parentalLeaves = pData || [];
  ...
  for (const p of currentPeople) {
    const isParental = parentalLeaves.some(pl => 
        pl.person_id === p.id && 
        new Date(pl.start_date) <= endDate && 
        (!pl.end_date || new Date(pl.end_date) >= startDate)
    );
    if (isParental && p.actualHours === 0) continue;
  ...
  console.log(`FINAL UI MATCH - UK Current Only: Count=${currentCount}, Expected=${currentExpected}, Actual=${currentActual}`);
}
run();
