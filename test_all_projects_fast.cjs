...
(async () => {
  ...
  for (const proj of projects) {
    ...
    await page.goto(`http://localhost:8080/projects/${proj.id}`, { waitUntil: 'domcontentloaded', timeout: 5000 }).catch(e => {
        // timeout is ok
    });
    ...
    if (!text || text.trim() === "") {
        console.log(`[${proj.id}] BLANK PAGE!`);
    }

    await page.close();
  }
  await browser.close();
  console.log("Done checking all projects");
})();
