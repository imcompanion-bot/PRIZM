...
const { data: project } = await supabase.from('projects').select('*, rate_cards(currency, name)').ilike('title', '%Wedding%').single();
...
// simulating project currency mode
...
const activeCurrency = "USD"; // let's assume it's USD project currency
...
console.log("officeCurrency:", officeCurrency);
...
timeEntries.forEach(te => {
    ...
    const cost = te.hours * convertCostToActiveCurrency(costPerHour, office);
    totalActualCost += cost;
});

console.log("Total Actual Cost (Project Currency Mode):", totalActualCost);
