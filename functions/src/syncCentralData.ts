...
async function sendFailureAlert(error: any, context: string): Promise<void> {
...
export async function runSync() {
  ...
  for (const row of projectsRows) {
    ...
    projectsBatchMap.set(projectId, {
      ...
      extra_data: {
        ...
        gp_fy20_21: parseNumber(row[151]),
        gp_fy21_22: parseNumber(row[152]),
        gp_fy22_23: parseNumber(row[153]),
        gp_fy23_24: parseNumber(row[179]),
        gp_fy24_25: parseNumber(row[202]),
        gp_fy25_26: parseNumber(row[252]),
        gp_fy26_27: parseNumber(row[261]),
        gp_q1_20_21: parseNumber(row[190]),
        gp_q2_20_21: parseNumber(row[191]),
        gp_q3_20_21: parseNumber(row[192]),
        gp_q4_20_21: parseNumber(row[193]),
        gp_q1_21_22: parseNumber(row[184]),
        gp_q2_21_22: parseNumber(row[185]),
        gp_q3_21_22: parseNumber(row[186]),
        gp_q4_21_22: parseNumber(row[187]),
        gp_q1_22_23: parseNumber(row[154]),
        gp_q2_22_23: parseNumber(row[155]),
        gp_q3_22_23: parseNumber(row[156]),
        gp_q4_22_23: parseNumber(row[157]),
        gp_q1_23_24: parseNumber(row[196]),
        gp_q2_23_24: parseNumber(row[197]),
        gp_q3_23_24: parseNumber(row[198]),
        gp_q4_23_24: parseNumber(row[199]),
        gp_q1_24_25: parseNumber(row[230]),
        gp_q2_24_25: parseNumber(row[231]),
        gp_q3_24_25: parseNumber(row[232]),
        gp_q4_24_25: parseNumber(row[233]),
        gp_q1_25_26: parseNumber(row[253]),
        gp_q2_25_26: parseNumber(row[254]),
        gp_q3_25_26: parseNumber(row[255]),
        gp_q4_25_26: parseNumber(row[256]),
        gp_q1_26_27: parseNumber(row[257]),
        gp_q2_26_27: parseNumber(row[258]),
        gp_q3_26_27: parseNumber(row[259]),
        gp_q4_26_27: parseNumber(row[260]),
      },
      created_at: new Date().toISOString(),
  ...
  if (projectsBatch.length > 0) {
    ...
    let projPage = 0;
    while (true) {
      const { data } = await supabase.from("projects" as any).select("id").range(projPage * 1000, (projPage + 1) * 1000 - 1);
  ...
  if (scopesBatch.length > 0) {
    ...
    const existingScopes: any[] = [];
    let scopePage = 0;
    while (true) {
      const { data } = await supabase.from("project_scopes" as any).select("id").range(scopePage * 1000, (scopePage + 1) * 1000 - 1);
      if (data && data.length > 0) {
...
export const syncCentralDataCron = onSchedule(
  {
    schedule: "0 6 * * *",
...
export const syncMonitorAgentCron = onSchedule(
  {
    schedule: "15 6 * * *",
    timeZone: "Europe/London",
    timeoutSeconds: 500,
  ...
  async (event) => {
    ...
    try {
      ...
      if (needsRetry) {
        ...
        await supabase.from("data_imports" as any).upsert(
          { dataset: "sync_monitor_status", last_imported_at: new Date().toISOString(), row_count: -1 } as any, // row_count -1 means retrying
          { onConflict: "dataset" } as any
        );
        ...
        if (syncSuccess) {
          ...
          await supabase.from("data_imports" as any).upsert(
            { dataset: "sync_monitor_status", last_imported_at: new Date().toISOString(), row_count: 1 } as any, // row_count 1 means retry succeeded
            { onConflict: "dataset" } as any
          );
        } else {
          ...
          await sendFailureAlert(
            lastSyncError || new Error("Sync failed after 3 forced retry attempts."),
            "Daily Monitor Agent Forced Auto-Recovery (failed after 3 attempts)"
          );

          await supabase.from("data_imports" as any).upsert(
            { dataset: "sync_monitor_status", last_imported_at: new Date().toISOString(), row_count: -2 } as any, // row_count -2 means failure
            { onConflict: "dataset" } as any
          );
        ...
        await supabase.from("data_imports" as any).upsert(
          { dataset: "sync_monitor_status", last_imported_at: new Date().toISOString(), row_count: 0 } as any, // row_count 0 means validated successfully first time
          { onConflict: "dataset" } as any
        );
      }
    } catch (err: any) {
      ...
      try {
        await supabase.from("data_imports" as any).upsert(
          { dataset: "sync_monitor_status", last_imported_at: new Date().toISOString(), row_count: -2 } as any, // row_count -2 means monitor error
          { onConflict: "dataset" } as any
        );
      } catch (upsertErr) {
...
export const syncCentralDataHttp = onRequest(
  {
    region: "us-east4",
...
export const syncCentralDataCallable = onCall(
  {
    region: "us-east4",
    timeoutSeconds: 500,
    memory: "1GiB",
