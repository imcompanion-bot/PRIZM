...
async function runSync() {
    ...
    const parseEfficiencies = (rows, type, startColOffset = 0) => {
        ...
        for (let i = headerRowIdx + 1; i < rows.length; i++) {
            ...
            for (const mc of monthCols) {
                ...
                if (amount && amount !== 0) {
                    results.push({
                        id: (0, uuid_1.v5)(`eff_${type}_${oppName}_${mc.monthDate}`, NAMESPACE),
    ...
    try {
        ...
        if (allEfficiencies.length > 0) {
            for (let i = 0; i < allEfficiencies.length; i += 100) {
                const { error } = await supabase.from("talent_efficiencies").upsert(allEfficiencies.slice(i, i + 100));
                if (error)
                    throw new Error(`Talent Efficiencies Upsert Error: ${error.message}`);
            }
        }
    }
    catch (e) {
