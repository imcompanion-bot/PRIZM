...
const replacement = `      // Proportion agency fee / scope to the portion of the project that overlaps the selected window
      ...
      let revenueInProjCurrency = 0;
      let revenueDisplay = 0;
      let totalScoped = 0;
      const effectiveScopedHoursByScopeId: Record<string, number> = {};

      // Helper for FX
      const convertRevToDisplay = (val: number, monthKey: string) => {
        if (p.project_currency === displayCurrency) return val;
        let gbpToUsd = fxRateUsd > 0 ? fxRateGbp / fxRateUsd : (fallbackGbpUsdRate || 1.27);
        if (monthlyFxRates[monthKey]) gbpToUsd = monthlyFxRates[monthKey];
        if (displayCurrency === "USD" && p.project_currency !== "USD") return val * gbpToUsd;
        if (displayCurrency === "GBP" && p.project_currency !== "GBP") return val / gbpToUsd;
        return val;
      };
      ...
      if (hasPhaseData) {
        for (const phase of projPhases) {
          ...
          for (const m of phaseMonths) {
            ...
            if (targetMonths.includes(monthKey)) {
              const revChunk = phaseFee * (overlapDays / totalPhaseDays);
              revenueInProjCurrency += revChunk;
              revenueDisplay += convertRevToDisplay(revChunk, monthKey);
        ...
        if (totalDays > 0) {
          ...
          for (const m of projMonths) {
            ...
            const monthKey = format(m, "yyyy-MM-01");
            if (targetMonths.includes(monthKey)) {
              const revChunk = fullRevenue * (overlapDays / totalDays);
              revenueInProjCurrency += revChunk;
              revenueDisplay += convertRevToDisplay(revChunk, monthKey);
      ...
      const revenue = revenueInProjCurrency;
`;
