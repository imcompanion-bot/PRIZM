...
async function run() {
  ...
  console.log('get_utilisation_summary:', res1.error || 'OK');
  
  const res2 = await supabase.rpc('get_project_costs_by_role', { _start_date: '2026-01-01', _end_date: '2026-07-31' });
  console.log('get_project_costs_by_role:', res2.error || 'OK');
}
run();
