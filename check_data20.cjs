...
async function run() {
  ...
  let pFrom = 0;
  while (true) {
     const { data } = await supabase.from('people').select('*').range(pFrom, pFrom + 999);
  ...
  console.log(`Current Only (Count=${currentCount}): Expected=${currentExpected}, Actual=${currentActual}`);
}
run();
